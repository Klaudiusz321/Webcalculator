...
def generate_output(g, Gamma, R_abcd, Ricci, Scalar_Curvature, G_upper, G_lower, n):
    ...
    # METRYKA
    lines_text.append("Metric tensor components:")
    lines_latex.append("Metric tensor components:")
    for i in range(n):
        for j in range(i, n):
            val = custom_simplify(g[i, j])
            if not val.equals(0):
                lines_text.append(f"g_({i}{j}) = {val}")
                lines_text.append(f"g_{{{i}{j}}} = \\({sp.latex(val)}\\)")
                lines_text.append("")
                
                # Dla LaTeX – możesz dodać etykiety np. "g_{ij} = ..."
                lines_latex.append(f"\\( g_{{{i}{j}}} = {sp.latex(val)} \\)")
                lines_latex.append("")
    
    # CHRISTOFFEL SYMBOLS
    lines_text.append("Non-zero Christoffel symbols:")
    lines_latex.append("Non-zero Christoffel symbols:")
    for a in range(n):
        for b in range(n):
            for c in range(n):
                val = custom_simplify(Gamma[a][b][c])
                if not val.equals(0):
                    lines_text.append(f"\\( \\Gamma^{{{a}}}_{{{b}{c}}} = {val} \\)")
                    lines_text.append(f"\\( \\Gamma^{{{a}}}_{{{b}{c}}} = {sp.latex(val)} \\)")
                    lines_text.append("")
                    
                    lines_latex.append(f"\\( \\Gamma^{{{a}}}_{{{b}{c}}} = {sp.latex(val)} \\)")
                    lines_latex.append("")
    
    # RIEMANN TENSOR
    lines_text.append("Non-zero Riemann tensor components:")
    lines_latex.append("Non-zero Riemann tensor components:")
    for a in range(n):
        for b in range(n):
            for c in range(n):
                for d in range(n):
                    ...
                    if not val.equals(0):
                        lines_text.append(f"\\( R_{{{a}{b}{c}{d}}} = {val} \\)")
                        lines_text.append(f"\\( R_{{{a}{b}{c}{d}}} = {sp.latex(val)} \\)")
                        lines_text.append("")
                        
                        lines_latex.append(f"\\( R_{{{a}{b}{c}{d}}} = {sp.latex(val)} \\)")
                        lines_latex.append("")
    
    # RICCI TENSOR
    lines_text.append("Non-zero Ricci tensor components:")
    lines_latex.append("Non-zero Ricci tensor components:")
    for i in range(n):
        for j in range(n):
            val = custom_simplify(Ricci[i, j])
            if not val.equals(0):
                lines_text.append(f"\\( R_{{{i}{j}}} = {val} \\)")
                lines_text.append(f"\\( R_{{{i}{j}}} = {sp.latex(val)} \\)")
                lines_text.append("")
                
                lines_latex.append(f"\\( R_{{{i}{j}}} = {sp.latex(val)} \\)")
                lines_latex.append("")
    
    # EINSTEIN TENSOR (dolne indeksy)
    lines_text.append("Non-zero Einstein tensor components (G_ij):")
    lines_latex.append("Non-zero Einstein tensor components (G_ij):")
    for i in range(n):
        for j in range(n):
    ...
    # SCALAR CURVATURE
    if not Scalar_Curvature.equals(0):
        lines_text.append("Scalar curvature R:")
        lines_text.append(f"\\( R = {Scalar_Curvature} \\)")
        lines_text.append(f"\\( R = {sp.latex(Scalar_Curvature)} \\)")
        lines_text.append("")
        
        lines_latex.append("Scalar curvature R:")
        lines_latex.append(f"\\( R = {sp.latex(Scalar_Curvature)} \\)")
        lines_latex.append("")
    
    # Łączymy linie w jeden ciąg
    output_text = "\n".join(lines_text)
    output_latex = "\n".join(lines_latex)
    
    return {
        "outputText": output_text,
        "outputLatex": output_latex
    }
